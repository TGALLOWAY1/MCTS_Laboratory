...
| L7 | Opponent modeling (blocking tracker, alliance, king-maker) | **Done (re-run)** — bugs fixed, features now active but no reliable competitive advantage |
...
### Layer 7 — Opponent modeling ← **DONE (re-run after bugfix)**
- [x] L7 original runs (2026-03-26) — Zero effect due to 3 implementation bugs (PR #102, commit `5346067`)
- [x] L7 bugfix re-run (2026-03-27) — 100 games across 4 runs with fixed code
  **Bugs fixed:** (1) `get_defensive_eval_adjustment()` wired into eval path, (2) `opponent_rollout_policy="same"` no longer short-circuits opponent model, (3) kingmaker occupancy threshold lowered 0.55→0.40.
  **Results:** Features now produce differentiated results (confirming bugs are fixed), but no reliable competitive advantage over L6 baseline.
  - Alliance detection: inconsistent — L7_alliance_only avg 24% vs L6_baseline avg 28%
  - Kingmaker detection: high variance — 20% (seed A) vs 34% (seed B)
  - Asymmetric rollouts: `self_heuristic_opp_random` is TrueSkill #1 across both seeds (29% avg win rate) but costs 2.4x compute
  **L7 conclusion**: Technique works but provides marginal benefit at 25 iter/move budget.
  Do NOT enable opponent modeling by default. See `archive/reports/layer7_bugfix_rerun_report.md`.
  ```bash
