...
interface PieceTrayProps {
  ...
  gameState?: any;
  /** Which player's pieces to display. Defaults to current_player if not set. */
  viewingPlayer?: string | null;
  onViewingPlayerChange?: (player: string) => void;
}
...
const ALL_PLAYERS = ['RED', 'BLUE', 'GREEN', 'YELLOW'];
...
export const PieceTray: React.FC<PieceTrayProps> = ({
  ...
  // Viewing player: parent-locked or falls back to current_player
  const displayPlayer = viewingPlayer ?? gameState?.current_player;
  const piecesUsed = gameState?.pieces_used?.[displayPlayer || ''] || [];
        ...
        <div className="px-3 py-1.5 border-b border-charcoal-700 flex items-center justify-center gap-1.5 flex-wrap">
          {ALL_PLAYERS.map(p => {
            ...
            return (
              <button
                key={p}
                onClick={() => onViewingPlayerChange(p)}
                className={`px-2 py-0.5 text-[10px] font-bold rounded-full transition-all ${isActive ? 'ring-2 ring-white scale-105' : 'opacity-50 hover:opacity-80'}`}
                style={{
                  backgroundColor: PLAYER_COLORS[p] + '33',
                  color: PLAYER_COLORS[p],
                  border: `1px solid ${PLAYER_COLORS[p]}`,
                }}
              >
                {p}
              </button>
          ...
          {availablePieces.map(pieceId => {
            ...
            return (
              <div
                key={pieceId}
                className={`
                group relative bg-charcoal-800 border rounded-md p-2 transition-all overflow-hidden
                ${isViewingOther
                    ? 'cursor-default border-charcoal-600'
                    : isSelected
                      ? 'cursor-pointer border-neon-blue ring-2 ring-neon-blue ring-opacity-50'
                      : isPieceUsed
                        ? 'border-charcoal-600 opacity-50 cursor-not-allowed'
                        : 'cursor-pointer border-charcoal-700 hover:border-charcoal-600 hover:bg-charcoal-750'
                  }
              `}
                onClick={() => {
                  if (isViewingOther || isPieceUsed) return;
