...
| **Layer 8** | Parallelization | Root-parallel multiprocessing, tree-parallel virtual loss. **Finding (throughput):** Near-linear speedup up to physical core count — 1.84x at 2 workers (92% efficiency), 3.13x at 4 workers (78% efficiency) on a 4-core machine. 8 workers is *slower* than 4 (oversubscription). Playing strength is flat at equal iteration budgets (mean scores 72–75 across all configs), suggesting parallelization provides speed but not quality gains. Playing strength test (root vs tree) pending. |
